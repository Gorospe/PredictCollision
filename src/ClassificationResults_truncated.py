...
df_ploeg = df_ploeg.drop(['numPlatCars', 'numHumanCars', 'spdLeader', 'BeaconIntervalPlat', 'BeaconIntervalHuman', 'DecelerationRate', 'PacketSize', 'waitTime'], axis=1)
...
f_names = ['DENMInterval', 'Ctr_Param(Ploegh||Spacing)']
...
rs_acc.to_excel('../results/resultsACC_rf.xlsx')

rs_cacc = cls.test_algorithms(X_cacc, y_cacc, cls.test_dec_tree)
rs_cacc.to_excel('../results/resultsLPF_rf_truncated.xlsx')

rs_ploeg = cls.test_algorithms(X_ploeg, y_ploeg, cls.test_dec_tree)
rs_ploeg.to_excel('../results/resultsPF_rf_truncated.xlsx')
